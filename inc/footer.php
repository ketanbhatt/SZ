<!--Footer Begins-->
		<footer>
			
			<div class = "container">
					<div class="footer-col-1">
						<nav>
							<ul>
								<li> <a href = "index.php">Home</a></li>
								<hr>
								<li> <a href = "products.php">Products</a></li>
								<hr>
								<li> <a href = "about.php">About</a></li>
								<hr>
								<li> <a href = "contact.php">Contact</a></li>

							</ul>
						</nav>
					</div>
					<div class="footer-col-2">
						<div class = "footer-buttons">
							<form target="_blank" method="get" action="download/SZintro.pdf">
								<button type="submit" name = "IntroLetter"> <img class = "downimg" src="img/download.png">Intro Letter</button>
							</form>
							<form target="_blank" method="get" action="download/SZclient.pdf">
								<button type="submit" name = "ClientList"> <img class = "downimg" src="img/download.png">Client List</button>
							</form>
						</div>

						
							<ul class="footer-images">
								<li> <img class = "footer-image" src = "img/emailicon.png" />   </li>
								<li> <img class = "footer-image" src = "img/phoneicon.png" />  </li>
								<li> <img class = "footer-image" src = "img/addicon.png" />  </li>
							</ul>
							
						<div class = "footer-contact">
							<ul>
								<li class = "first-contact"> jcbhatt@supplyzone.com </li>
								<li class = "second-contact">+91-7838970330 </li>
								<li class = "third-contact"> C-31/Y-1, MIG Flats Dilshad Garden, Delhi-95</li>
 
							</ul>
						</div>
					</div>
			</div>
			<div style="clear:both;"></div>
			<div class="copyright-bar">
				<p>&copy; Copyright 2015 Supply Zone. All Rights Reserved</p>
			</div>
		</footer>

	</body>
</html>